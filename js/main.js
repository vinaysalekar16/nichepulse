...
function renderBlogs(filter = 'all') {
  blogsGrid.innerHTML = '';
  const filtered = filter === 'all' ? DB.blogs : DB.blogs.filter(b => b.niche === filter);

  if (filtered.length === 0) {
    blogsGrid.innerHTML = `<div class="empty-state" style="grid-column:1/-1"><p>No articles yet. <a href="admin/index.html">Add one in the admin panel →</a></p></div>`;
    return;
  }
